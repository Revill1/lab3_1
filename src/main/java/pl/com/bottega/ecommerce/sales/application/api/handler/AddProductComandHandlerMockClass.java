...
public class AddProductComandHandlerMockClass {

	private static Product product = new Product(new Id("1"), new Money(
			new BigDecimal(333), Currency.getInstance(Locale.getDefault())),
			"instance", ProductType.STANDARD);
	private static Id id = new Id("1");
	private static Client client = new Client();

	public static ProductRepository productRepositoryMock() {
		ProductRepository productRepository = Mockito
	...
	public static SuggestionService suggestionServiceMock() {
		...
		Mockito.when(suggestionService.suggestEquivalent(product, client))
				.thenReturn(product);
	...
	public static ClientRepository clientRepositoryMock() {
		ClientRepository clientRepository = Mockito
